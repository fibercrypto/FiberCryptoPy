...
import platform
import sys
from fibercryptopy import __version__
...
class fibercrypto_build_ext(build_ext, object):
    def build_extension(self, ext):

        if ext.name != "_fibercryptopy":
            # Handle unspecial extensions with the parent class' method
            super(fibercrypto_build_ext, self).build_extension(ext)
        else:
...
fcpath = path.join(*("gopath/src/github.com/fibercrypto/libfibercrypto".split("/")))
lib_path = path.join(fcpath, "build", "libfibercrypto")
library_file = path.join(lib_path, "libfibercrypto.a")
extra_link_args = []
...
setup(
    name='fibercryptopy',  # Required
    version=__version__,  # Required
    description="FiberCrypto Python Library",
    long_description=long_description,
    long_description_content_type="text/markdown",
    url="https://github.com/fibercrypto/FiberCryptoPy",
    ...
    keywords="fibercryptopy crypto coin currency blockchain",  # Optional
    py_modules=["fibercryptopy"],
    packages=find_packages(exclude=["contrib", "docs", "tests"]),  # Required
    install_requires=[],
    extras_require={"dev": ["check-manifest"], "test": ["coverage"]},  # Optional
    package_data={},
    entry_points={"console_scripts": []},
    cmdclass={"build_ext": fibercrypto_build_ext},
    ext_modules=[
        Extension(
            "_fibercryptopy",
            ["swig/fibercryptopy_wrap.c"],
            include_dirs=["swig/include", path.join(fcpath, "include")],
